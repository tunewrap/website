...
if(!indexSource.includes('<meta name="tunewrap-build" content="12.14.12">')&&
   !indexSource.includes('<meta name="tunewrap-build" content="12.14.13">')){
  throw new Error('Сначала требуется установленный Stage 12.14.12. Файлы не изменены.');
}
if(!sources.get('admin/site.js').includes('async function translateResilient')||
   !sources.get('admin/site.js').includes('const failedFields=[]')||
   !sources.get('admin/site.html').includes('/admin/site.js?v=12.14.13')){
  throw new Error('Файлы Stage 12.14.13 не распакованы поверх website. Файлы не изменены.');
}

function replaceOnce(source,before,after,label){
  if(source.includes(after))return source;
  const count=source.split(before).length-1;
  if(count!==1)throw new Error(`${label}: ожидаемый Stage 12.14.12 не найден. Файлы не изменены.`);
...
controlledPullTest=replaceOnce(controlledPullTest,
  '/<meta name="tunewrap-build" content="12\\.14\\.(?:11|12)">/',
...
announcementTest=replaceOnce(announcementTest,
  '/\\/admin\\/site\\.js\\?v=12\\.14\\.(?:8|12)/',
  '/\\/admin\\/site\\.js\\?v=12\\.14\\.(?:8|12|13)/',
  'Stage 12.14.8 Admin asset compatibility');

const pkg=JSON.parse(sources.get('package.json'));
pkg.scripts||={};
pkg.scripts['sitefieldrecovery:test']='node scripts/stage-12.14.13-site-field-translation-recovery-test.js';
if(typeof pkg.scripts.test!=='string')throw new Error('package scripts.test отсутствует. Файлы не изменены.');
if(!pkg.scripts.test.includes('sitefieldrecovery:test'))pkg.scripts.test+=' && npm run sitefieldrecovery:test';
...
console.log('PASS: Stage 12.14.13 Site Field Translation Recovery installed.');
console.log('PASS: failed translation batches are divided until the exact bad field is isolated.');
console.log('PASS: valid UA/GE fields are committed even when another field fails.');
console.log('PASS: EN/UA/GE/DE remain independent and Admin reports exact field counts.');
